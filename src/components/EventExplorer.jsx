...
export default function EventExplorer({ onSelectEvent, onJoinClick }) {
  ...
  const { filteredEvents, pendingCount, pastCount, count2026, count2027 } = useMemo(() => {
    let pending = [];
    let past = [];
    let c2026 = 0;
    let c2027 = 0;
    ...
    const filtered = combined.filter(event => {
      ...
      return matchYear && matchSearch && matchDistance && matchRegion;
    });

    // Season-scoped pending & past counts for status buttons
  ...
  }, [searchTerm, selectedDistance, selectedRegion, statusFilter, selectedYear]);

  return (
    <section id="events" className="py-24 bg-[#040b17] relative">
      <div id="events-container" className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <div id="events-header" className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
          <div id="events-header-text">
            <div id="events-header-eyebrow" className="inline-flex items-center space-x-2 text-volt text-xs sm:text-sm font-athletic font-bold uppercase tracking-widest mb-2">
              <span className="w-8 h-0.5 bg-volt" />
              <span>Provincial Series Calendar</span>
            </div>
            <h2 id="events-title" className="text-3xl sm:text-5xl font-athletic font-bold uppercase tracking-tight text-white">
              Nova Scotia Road <span className="text-volt">Race Calendar</span>
            </h2>
            <p id="events-subtitle" className="text-slate-400 text-sm sm:text-base mt-2 max-w-2xl">
              Sanctioned events across <strong className="text-white">2026 and 2027</strong>. 
              Ordered with <strong className="text-volt">still pending / upcoming races first</strong>, 
          ...
          <div id="events-header-badges" className="flex flex-wrap items-center gap-2 text-xs uppercase font-athletic tracking-wider">
            <span id="badge-upcoming-count" className="px-3 py-1.5 bg-volt/15 text-volt border border-volt/40 rounded-lg flex items-center space-x-1.5 font-bold">
              <Flame className="w-3.5 h-3.5 text-volt" />
              <span>{pendingCount} Upcoming</span>
            </span>
            <span id="badge-2026-count" className="px-3 py-1.5 bg-dark-800 text-slate-300 border border-dark-700 rounded-lg flex items-center space-x-1.5 font-semibold">
              <span>{count2026} Races in 2026</span>
            </span>
            <span id="badge-2027-count" className="px-3 py-1.5 bg-dark-800 text-volt border border-dark-700 rounded-lg flex items-center space-x-1.5 font-semibold">
        ...
        {/* Filter & Search Bar */}
        <div id="events-filter-bar" className="bg-dark-850 border border-dark-750 p-4 sm:p-5 rounded-2xl mb-10 shadow-2xl space-y-4">
          
          {/* Row 1: Year Selector & Status Filter Tabs */}
          <div id="events-filter-row-1" className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-dark-750">
            
            {/* Season Selector */}
            <div id="events-season-selector" className="flex items-center space-x-2">
              <span className="text-xs uppercase tracking-wider font-athletic text-slate-400 font-semibold mr-1 flex items-center space-x-1">
                <Layers className="w-3.5 h-3.5 text-volt" />
                <span>Season:</span>
              </span>
              {yearFilters.map((y) => (
                <button
            ...
            <div id="events-status-selector" className="flex items-center space-x-2">
              <span className="text-xs uppercase tracking-wider font-athletic text-slate-400 font-semibold mr-1">
                Status:
              </span>
              <button
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-athletic uppercase tracking-wider font-semibold transition-all flex items-center space-x-1.5 cursor-pointer ${
                  statusFilter === 'PENDING'
                    ? 'bg-volt text-black shadow-md font-bold'
                    : 'bg-dark-800 text-volt hover:text-white border border-dark-700'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-athletic uppercase tracking-wider font-semibold transition-all cursor-pointer ${
                  statusFilter === 'PAST'
                    ? 'bg-dark-700 text-white border border-dark-600 shadow-md font-bold'
          ...
          <div id="events-filter-row-2" className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div id="events-search-wrapper" className="md:col-span-6 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-dark-900 border border-dark-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-volt transition-colors"
              />
              {searchTerm && (
                <button 
            ...
            <div id="events-region-wrapper" className="md:col-span-6 flex items-center space-x-2">
              <MapPin className="w-4 h-4 text-volt flex-shrink-0 hidden sm:block" />
              <select
                id="events-region-select"
                value={selectedRegion}
                onChange={(e) => setSelectedRegion(e.target.value)}
                className="w-full px-3 py-2.5 bg-dark-900 border border-dark-700 rounded-lg text-sm text-white focus:outline-none focus:border-volt transition-colors"
              >
                {regionFilters.map((r) => (
                  <option key={r.value} value={r.value} className="bg-dark-900 text-white">
                    {r.label}
                  </option>
                ))}
              </select>
          ...
          {/* Row 3: Distance Filter Badges */}
          <div id="events-distance-filters" className="flex flex-wrap items-center gap-2 pt-1 border-t border-dark-800">
            <span className="text-xs uppercase tracking-wider text-slate-400 font-athletic mr-2 flex items-center space-x-1">
              ...
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-athletic uppercase tracking-wider font-semibold transition-all cursor-pointer ${
                  selectedDistance === dist
                    ? 'bg-volt text-black shadow-md'
                    : 'bg-dark-800 text-slate-300 hover:text-white hover:bg-dark-750 border border-dark-700'
                }`}
              >
                {dist}
              </button>
        ...
        {filteredEvents.length === 0 ? (
          <div id="events-empty-state" className="bg-dark-850 border border-dark-750 rounded-2xl p-12 text-center max-w-lg mx-auto shadow-2xl">
            <p className="text-slate-300 font-athletic text-lg uppercase">No races match your filters</p>
            <p className="text-xs text-slate-500 mt-1">Try resetting the year, status, or search terms.</p>
            <button
              ...
              className="mt-4 px-4 py-2 bg-volt hover:bg-[#e5d800] text-black text-xs font-athletic font-bold uppercase tracking-wider rounded cursor-pointer shadow-[0_0_20px_rgba(254,240,0,0.35)]"
            >
              Reset All Filters
            </button>
          </div>
            ...
            {filteredEvents.map((race) => {
              ...
              return (
                <div 
                  ...
                  className={`group relative rounded-2xl overflow-hidden shadow-xl transition-all duration-300 flex flex-col justify-between transform hover:-translate-y-1 border cursor-pointer ${
                    pending 
                      ? 'bg-dark-850 hover:bg-dark-800 border-volt/60 hover:border-volt shadow-[0_0_25px_rgba(254,240,0,0.22)]' 
                      : 'bg-dark-900/90 hover:bg-dark-850 border-dark-750/80 opacity-90 hover:opacity-100 shadow-lg'
                  }`}
                >
                  ...
                  <div id={`event-card-media-${race.id}`} className="relative h-48 overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#040b17]/85 via-transparent to-transparent" />
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] uppercase font-athletic tracking-wider font-semibold text-volt">
                          {race.type}
                        </span>
                    ...
                    <div className="space-y-2 pt-2 border-t border-dark-750">
                          ...
                          {race.distances.map((dist, i) => (
                            <span 
                              ...
                              className={`px-2 py-0.5 text-[11px] font-mono rounded ${
                                pending 
                                  ? 'bg-dark-900 border border-dark-700 text-volt' 
                                  : 'bg-dark-950 border border-dark-750 text-slate-400'
                              }`}
                            >
                              {dist}
                            </span>
                      ...
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-400">Course / Elevation:</span>
                        <span className="text-slate-300 font-medium text-right text-[11px]">{race.elevation}</span>
                      </div>

                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-400">Status:</span>
                        {pending ? (
                          <span className="text-volt font-semibold flex items-center space-x-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                      <button
                        ...
                        className="w-full py-2.5 px-3 bg-dark-750 hover:bg-dark-700 text-white hover:text-volt border border-dark-650 rounded text-xs font-athletic uppercase tracking-wider transition-colors flex items-center justify-center space-x-1 cursor-pointer"
                      >
                        <span>Details</span>
                      ...
                      {pending ? (
                        <a
                          ...
                          className="w-full py-2.5 px-3 bg-volt hover:bg-[#e5d800] text-black font-athletic font-bold rounded text-xs uppercase tracking-wider transition-colors flex items-center justify-center space-x-1 shadow-[0_0_20px_rgba(254,240,0,0.35)] cursor-pointer"
                        >
                          <span>Register</span>
                        ...
                        <a
                          ...
                          rel="noopener noreferrer"
                          onClick={(e) => e.stopPropagation()}
                          className="w-full py-2.5 px-3 bg-dark-800 hover:bg-dark-700 text-slate-300 hover:text-white font-athletic font-semibold rounded text-xs uppercase tracking-wider transition-colors flex items-center justify-center space-x-1 border border-dark-700 cursor-pointer"
                        >
                          <span>View Results</span>
                          <Trophy className="w-3 h-3 text-volt" />
                        </a>
                      )}
                    </div>
