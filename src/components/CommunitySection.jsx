...
export default function CommunitySection() {
  ...
  return (
    <section id="community-section" className="py-24 bg-[#0b1627] border-y border-dark-750 relative">
      ...
      <div id="community-container" className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <div id="community-header" className="text-center max-w-3xl mx-auto mb-14 space-y-3">
          <div id="community-badge" className="inline-flex items-center space-x-2 text-volt text-xs sm:text-sm font-athletic font-bold uppercase tracking-widest">
            <span className="w-6 h-0.5 bg-volt" />
            <span>Community & Leadership</span>
            <span className="w-6 h-0.5 bg-volt" />
          </div>
          <h2 id="community-title" className="text-3xl sm:text-5xl font-athletic font-bold uppercase tracking-tight text-white">
            The Nova Scotia <span className="text-volt">Running Network</span>
          </h2>
          <p id="community-description" className="text-slate-400 text-sm sm:text-base">
        ...
        <div id="community-tabs" className="flex justify-center mb-10">
          <div className="bg-dark-850 p-1.5 rounded-xl border border-dark-750 flex space-x-2">
            <button
        ...
        {activeTab === 'clubs' && (
          <div id="community-clubs-view" className="space-y-8">
            <div id="community-clubs-grid" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {runClubsData.map((club, idx) => (
                <div 
                  key={idx}
                  id={`club-card-${idx + 1}`}
                  className="bg-dark-850 border border-dark-750 hover:border-volt/60 rounded-2xl p-6 transition-all duration-300 space-y-4 hover:-translate-y-1"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-xl font-athletic font-bold text-white uppercase tracking-wide">
                        {club.name}
                      </h3>
                      <div className="flex items-center space-x-1.5 text-xs text-volt font-semibold mt-1">
                        <MapPin className="w-3.5 h-3.5" />
                        <span>{club.location}</span>
                      </div>
                    </div>
                    <div className="w-9 h-9 rounded-lg bg-dark-750 text-volt flex items-center justify-center">
                      <Users className="w-4 h-4" />
                    </div>
                  </div>

                  <div className="space-y-2 text-xs text-slate-300 pt-2 border-t border-dark-750">
                    <div>
                      <span className="text-slate-400 block">Weekly Runs:</span>
                      <span className="text-slate-200 font-medium">{club.meets}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">Club Focus:</span>
                      <span className="text-slate-200">{club.focus}</span>
                    </div>
                  </div>

                  <div className="pt-2">
                    <span className="text-xs text-volt font-mono font-semibold flex items-center space-x-1">
            ...
            <div id="community-club-partner-cta" className="bg-dark-850 border border-dark-750 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="space-y-1 text-center sm:text-left">
                <h4 className="font-athletic text-lg uppercase text-white font-bold">
          ...
          <div id="community-board-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {boardMembersData.map((member, idx) => (
              <div 
                ...
                className="bg-dark-850 border border-dark-750 rounded-2xl overflow-hidden group hover:border-volt/60 transition-all"
              >
                ...
                <div className="p-5 space-y-2">
                  <h4 className="text-lg font-athletic font-bold text-white uppercase tracking-wide">
                    {member.name}
                  </h4>
        ...
        {activeTab === 'life' && (
          <div id="community-life-members-view" className="bg-dark-850 border border-dark-750 rounded-2xl p-8 sm:p-12 space-y-8 shadow-2xl">
            <div className="max-w-2xl">
              <span className="text-xs uppercase font-athletic font-bold text-volt tracking-widest block mb-1">
                Hall of Distinction
              </span>
            ...
            <div id="community-life-members-grid" className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {lifeMembers.map((name, idx) => (
                <div 
                  ...
                  className="bg-dark-900 border border-dark-750 p-4 rounded-xl text-center hover:border-volt transition-colors"
                >
                  <div className="w-8 h-8 rounded-full bg-volt/10 text-volt mx-auto flex items-center justify-center font-bold text-xs mb-2">
                    ★
                  </div>
                  <div className="text-sm font-athletic font-bold text-white uppercase tracking-wide">
                    {name}
                  </div>
                  <div className="text-[10px] text-volt font-semibold tracking-wider uppercase mt-0.5">
                    Life Member
                  </div>
                </div>
              ))}
